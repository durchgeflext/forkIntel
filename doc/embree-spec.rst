...
All API calls carry the prefix ``rtc`` (or ``RTC`` for types) which
stands for **r**\ ay **t**\ racing **c**\ ore. The API supports scenes
consisting of different geometry types such as triangle meshes, quad
meshes (triangle pairs), grid meshes, flat curves, round curves,
oriented curves, subdivision meshes, instances, and user-defined
geometries. See Section `Scene Object <#scene-object>`__ for more
information.
...
and ray packets. See Section `Ray Queries <#ray-queries>`__ for more
information.
...
`rtcNewDevice <#rtcnewdevice>`__ function (or
`rtcNewSYCLDevice <#rtcnewsycldevice>`__ when using SYCL for the GPU).
...
all devices by invoking `rtcReleaseDevice <#rtcreleasedevice>`__. An
...
for a complete example of how to trace single rays and ray packets.
...
`rtcInitIntersectArguments <#rtcinitintersectarguments>`__ and
`rtcInitOccludedArguments <#rtcinitoccludedarguments>`__ for more
...
`rtcNewSYCLDevice <#rtcnewsycldevice>`__
...
`rtcSetDeviceSYCLDevice <#rtcsetdevicesycldevice>`__ function.
...
argument) please see the `rtcNewDevice <#rtcnewdevice>`__ function
description.
...
`rtcSYCLDeviceSelector <#rtcsycldeviceselector>`__
...
`rtcIsSYCLDeviceSupported <#rtcissycldevicesupported>`__
...
context again to -1. The ``instID`` field is copied automatically by
each primitive intersector into the ``instID`` field of the hit
structure when the primitive is hit. See the `User
...
factor using the
`rtcSetGeometryMaxRadiusScale <#rtcsetgeometrymaxradiusscale>`__
...
`rtcInitRayQueryContext <#rtcinitrayquerycontext>`__
...
``RTCIntersectArguments`` or ``RTCOccludedArguments`` to the intersect
and occluded queries. If enable is true the argument filter function
...
`rtcSetSceneFlags <#rtcsetsceneflags>`__ for more details.
...
`rtcInitIntersectArguments <#rtcinitintersectarguments>`__,
`rtcInitOccludedArguments <#rtcinitoccludedarguments>`__,
`rtcSetSceneFlags <#rtcsetsceneflags>`__
...
`rtcInvokeOccludedFilterFromGeometry <#rtcinvokeoccludedfilterfromgeometry>`__,
`rtcSetGeometryIntersectFunction <#rtcsetgeometryintersectfunction>`__
...
`rtcInvokeIntersectFilterFromGeometry <#rtcinvokeintersectfilterfromgeometry>`__,
`rtcSetGeometryOccludedFunction <#rtcsetgeometryoccludedfunction>`__
...
intersected instance is stored in the ``instID`` field of the ray query
context, which must be deep copied into the ``instID`` member of the
hit.
...
callback function, even if the order of rays inside a ray packet has
changed.
...
`rtcIntersect1 <#rtcintersect1>`__,
`rtcIntersect4/8/16 <#rtcintersect4816>`__,
`rtcOccluded1 <#rtcoccluded1>`__,
...
The ``flags`` member can get used to enable special traversal mode.
Using the ``RTC_RAY_QUERY_FLAG_INCOHERENT`` flag uses an optimized
traversal algorithm for incoherent rays (default), while
``RTC_RAY_QUERY_FLAG_COHERENT`` uses an optimized traversal algorithm
for coherent rays (e.g. primary camera rays).
...
`RTCFeatureFlags <#rtcfeatureflags>`__ for a more detailed description.
...
`rtcInitRayQueryContext <#rtcinitrayquerycontext>`__ for more details.
...
geometry using the ``RTC_RAY_QUERY_FLAG_INVOKE_ARGUMENT_FILTER`` ray
...
`rtcIntersect1 <#rtcintersect1>`__,
`rtcIntersect4/8/16 <#rtcintersect4816>`__,
`RTCFeatureFlags <#rtcfeatureflags>`__,
`rtcInitRayQueryContext <#rtcinitrayquerycontext>`__,
`RTC_GEOMETRY_TYPE_USER <#rtc_geometry_type_user>`__,
`rtcSetGeometryMaxRadiusScale <#rtcsetgeometrymaxradiusscale>`__
...
cases, see section
`rtcInitIntersectArguments <#rtcinitintersectarguments>`__ for more
details.
...
flags to ``0`` (``flags`` ray member). The ray time (``time`` ray
member) must be initialized to a value in the range $[0, 1]. The ray
segment has to be in the range :math:`[0, \infty]`, thus ranges that
start behind the ray origin are not valid, but ranges can reach to
infinity. See Section `RTCRay <#rtcray>`__ for the ray layout
description.
...
`rtcOccluded1 <#rtcoccluded1>`__,
`rtcIntersect4/8/16 <#rtcintersect4816>`__, `RTCRayHit <#rtcrayhit>`__,
`rtcInitIntersectArguments <#rtcinitintersectarguments>`__
...
member) must be initialized to a value in the range :math:`[0, 1]`. The
ray segment must be in the range :math:`[0, \infty]`, thus ranges that
start behind the ray origin are not valid, but ranges can reach to
infinity. See Section `RTCRay <#rtcray>`__ for the ray layout
description.
...
`rtcIntersect1 <#rtcintersect1>`__,
`rtcOccluded4/8/16 <#rtcoccluded4816>`__, `RTCRay <#rtcray>`__,
`rtcInitOccludedArguments <#rtcinitoccludedarguments>`__
...
packet. The passed optional arguments struct (``args`` argument) are
used to pass additional arguments for advanced features. See Section
`rtcIntersect1 <#rtcintersect1>`__ for more details and a description of
...
into filter functions or user geometry callbacks. Under some conditions
the application can assume packets to stay intakt, which can determined
by querying the ``RTC_DEVICE_PROPERTY_NATIVE_RAY4_SUPPORTED``,
...
section `rtcInitOccludedArguments <#rtcinitoccludedarguments>`__ for
more details. See Section `rtcOccluded1 <#rtcoccluded1>`__ for more
...
transformed ray (``ray`` argument) into a scene (``scene`` argument)
...
`rtcIntersect1 <#rtcintersect1>`__, `RTCRay <#rtcray>`__
...
transformed ray packet (``ray`` argument) into a scene (``scene``
...
the ``rtcIntersect4/8/16`` function. The callback arguments structure of
the callback invokation has to get passed to the ray forwarding
(``args`` argument). The user geometry callback should instantly
terminate after invoking the ``rtcForwardIntersect4/8/16`` function.
...
argument) from a user geometry callback. The function can only get
invoked from a user geometry callback for a ray traversal initiated with
the ``rtcOccluded4/8/16`` function. The callback arguments structure of
the callback invokation has to get passed to the ray forwarding
(``args`` argument). The user geometry callback should instantly
terminate after invoking the ``rtcForwardOccluded4/8/16`` function.
